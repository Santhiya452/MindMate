...
function Register() {
  ...
  const handleKey = (e) => {
    if (e.key === 'Enter') step === 1 ? handleRegister() : handleVerifyOtp()
  }
      ...
      <div style={styles.card}>
        ...
        <div style={styles.stepWrap}>
          <div style={styles.stepItem}>
            <div style={{ ...styles.stepDot, background: 'linear-gradient(135deg, #7C3AED, #22D3EE)' }} />
            <span style={{ ...styles.stepText, color: '#22D3EE' }}>Account</span>
          </div>
          <div style={styles.stepLine} />
          <div style={styles.stepItem}>
            <div style={{ ...styles.stepDot, background: step === 2 ? 'linear-gradient(135deg, #7C3AED, #22D3EE)' : 'rgba(255,255,255,0.1)' }} />
            <span style={{ ...styles.stepText, color: step === 2 ? '#22D3EE' : '#475569' }}>Verify</span>
          </div>
        </div>
        ...
        <p style={styles.subtitle}>
          {step === 1 ? 'Join MindMate and start your journey' : `We sent a code to ${email}`}
        </p>
          ...
          <>
            <div style={styles.fieldWrap}>
              ...
              <input type='text' placeholder='Your name' value={name}
                onChange={e => setName(e.target.value)} onKeyPress={handleKey}
                style={styles.input} />
            </div>
            <div style={styles.fieldWrap}>
          ...
          <>
            <div style={styles.fieldWrap}>
              ...
              <input type='text' placeholder='000000' value={otp} maxLength={6}
                onChange={e => setOtp(e.target.value)} onKeyPress={handleKey}
                style={{ ...styles.input, textAlign: 'center', fontSize: '24px', letterSpacing: '12px' }} />
            </div>
            <button onClick={handleVerifyOtp} disabled={loading}
              style={{ ...styles.btn, opacity: loading ? 0.7 : 1 }}>
              {loading ? 'Verifying...' : 'Verify & Create Account ✓'}
            </button>
            <button onClick={() => { setStep(1); setError(''); setSuccess('') }} style={styles.backBtn}>
              ← Go back
            </button>
          </>
        )}
...
const styles = {
  ...
  logoWrap: {
    ...
    marginBottom: '24px', gap: '6px',
  },
  brand: { fontSize: '28px', fontWeight: '800', letterSpacing: '-0.5px' },
  tagline: { fontSize: '12px', color: '#CBD5E1', textAlign: 'center', margin: 0 },
  stepWrap: {
  ...
  stepDot: {
    width: '12px', height: '12px', borderRadius: '50%', transition: 'background 0.3s',
  },
  stepText: { fontSize: '11px', fontWeight: '600', letterSpacing: '0.5px' },
  stepLine: { width: '60px', height: '2px', background: 'rgba(124,58,237,0.3)', marginBottom: '16px' },
  title: { fontSize: '22px', fontWeight: '700', color: '#fff', margin: '0 0 4px 0' },
  subtitle: { fontSize: '13px', color: '#64748B', margin: '0 0 20px 0' },
  error: {
  ...
  backBtn: {
    ...
    color: '#94A3B8', fontSize: '13px', cursor: 'pointer', marginTop: '8px',
...
// 👑 CRITICAL EXPORT LINE VITE WAS ASKING FOR:
